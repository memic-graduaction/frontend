export { ReactComponent as Logo } from '../assets/Logo.svg';
export { ReactComponent as Switch } from '../assets/Switch.svg';
export { ReactComponent as BookMark } from '../assets/BookMark.svg';
export { ReactComponent as Close } from '../assets/Close.svg';
export { ReactComponent as Microphone } from '../assets/Microphone.svg';
export { ReactComponent as Stop } from '../assets/Stop.svg';
export { ReactComponent as PlayBtn } from '../assets/PlayBtn.svg';
